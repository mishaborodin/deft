...
class SSOCookies(object):
    def __init__(self, url, krb=True, pem_cert_file_path=None, pem_cert_key_path=None, cert_key_password=None,
        ...
        if krb:
            self.curl.setopt(self.curl.USERAGENT, self.user_agent_krb)
            self.curl.setopt(self.curl.HTTPAUTH, self.curl.HTTPAUTH_GSSNEGOTIATE)
            ...
            if cert_key_password:
                self.curl.setopt(self.curl.SSLKEYPASSWD, cert_key_password)
